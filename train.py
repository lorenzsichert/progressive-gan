...
if load_ckpt:
    try:
        generator.load_state_dict(torch.load(f"ckpt/G-{layer}-a{counting_alpha}.pth"))
        discriminator.load_state_dict(torch.load(f"ckpt/D-{layer}-a{counting_alpha}.pth"))
    except:
...
for ep in range(n_epochs):
    ...
    for batch in dataloader:
        ...
        if i % sample_interval == 0:
            ...
            with torch.no_grad():
                fixed = generator(fixed_noise, alpha)
